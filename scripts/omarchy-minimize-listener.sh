...
while true; do
  ...
  case "$line" in
    workspace*|activeworkspace*)
      ...
      elif [[ "$ws" == "$TARGET_WS" ]]; then
        # Immediately bounce away from the minimize workspace
        if [[ -f "$LAST_WS_FILE" ]]; then
          back_ws="$(cat "$LAST_WS_FILE")"
        else
          back_ws="1"
        fi
        hyprctl dispatch workspace "$back_ws" >/dev/null 2>&1 || true
        echo "bounced from $TARGET_WS to $back_ws" >> "$LOG_FILE"
      fi
      ;;
